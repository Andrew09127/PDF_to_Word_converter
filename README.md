...
|---|---|---|---|
| **Сканы (LLM)** | страницы-изображения, текст не выделяется | Docling + RapidOCR (+ опц. LLM) | медленнее |
| **Нативный PDF** | есть текстовый слой (текст копируется) | pdf2docx | быстро |

В скан-режиме распознаётся текст, восстанавливается структура (заголовки,
абзацы, таблицы метка|значение, шапка сторон, штамп ЭП), чистятся типичные
OCR-ошибки. Нативный режим переносит текст/таблицы/картинки напрямую.
